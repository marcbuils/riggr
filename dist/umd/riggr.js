(function (global, factory) {
  if (typeof define === 'function' && define.amd) {
    define(['exports', 'module', 'knockout', 'jquery', 'observer', 'router'], factory);
  } else if (typeof exports !== 'undefined' && typeof module !== 'undefined') {
    factory(exports, module, require('knockout'), require('jquery'), require('observer'), require('router'));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, mod, global.ko, global.$, global.observer, global.router);
    global.riggr = mod.exports;
  }
})(this, function (exports, module, _knockout, _jquery, _observer, _router) {
  var _interopRequire = function (obj) { return obj && obj.__esModule ? obj['default'] : obj; };

  // Loops through and loads routes, sets app properties
  module.exports = rigg;

  var _ko = _interopRequire(_knockout);

  var _$ = _interopRequire(_jquery);

  var _observer2 = _interopRequire(_observer);

  var _router2 = _interopRequire(_router);
  ...
  var loadView = function (view, controller, args, load) {
    ...
    // Transition-out
    _$(el).fadeTo(transition, 0, function () {
      // Set html
      _$(el).html(view);
      // Bind it up
      _ko.cleanNode(el);
      _ko.applyBindings(controller, el);
      // Process transition-in
      _$(this).fadeTo(transition, 1);
      // Fire load
      if (load) {
        controller.load.apply(controller, args);
      }
      // Publish onRoute
      _observer2.publish('onRoute');
    });
    // Set page title
  ...
  var registerObservables = function (vm) {
    ...
    for (var obsName in vm.observables) {
      ...
      if (_ko.isObservable(vm[obsName])) {
        vm[obsName](value);
      } else {
  ...
  var build = function (route, path) {
    rigg.moduleLoader([paths.controllers + '/' + path], function (controller) {
      ...
      applyLibs(controller, function () {
        ...
        // Create route
        _router2.on(route, routeHandler);
  ...
  var loadApp = function () {
    rigg.moduleLoader([paths.controllers + '/app'], function (app) {
      rigg.templateLoader([paths.views + '/app.html'], function (appView) {
        // Load view into main
        _$('#' + appContainer).html(appView);
        // Apply app bindings
        _ko.applyBindings(app);
        // Listen for route change
        _observer2.subscribe('onRoute', function () {
